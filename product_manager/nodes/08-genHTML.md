---
name: 08-genHTML
description: 在 genPrototype 节点产出确认后进入。在此节点基于设计稿生成高保真HTML原型和入口页面，产出可预览的HTML原型包。
---
...
读取 detail 节点的产出物：
- 从 Memory.json 获取 detail 的 `output.path`
...
**更新规则：**
1. 读取当前PRD文档
2. 如果PRD中不存在第5章节"原型与交互"，则在文档末尾追加该章节
3. 如果PRD中已存在第5章节，则替换该章节内容
4. 章节内容包含原型索引和页面原型引用路径
...
📦 文件地址：{项目路径}/output/{Version}/prototype.zip
...
7. 打包HTML文件为zip
8. 调用 manage.update_node_output("genHTML", {"type": "html_bundle", "path": "{项目名称}_{datetime}/output/{Version}/prototype/"})
9. 展示产出摘要（文件地址+页面列表）
10. **用户验收标准**
    ...
    **重要规则：**
    - **只有用户明确表达验收通过意图时，才能进入 change 节点或项目完成**
    - **任何非验收意图（修改、补充、疑问、部分确认、犹豫）都必须停留在 genHTML 节点，不得进入下一节点**
    - 用户表达"变更"、"加功能"、"改需求"等意图时，进入 change 节点

11. 等待用户确认
12. 确认后 → 调用 manage.update_node_status("genHTML", "CONFIRM")

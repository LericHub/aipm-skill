...
- 7节点标准流程：ROUTER → BRAINSTORM → CLARIFY → ANALYSIS → DETAIL → GENHTML → CHANGE
...
- **05 DETAIL** (`nodes/05-detail.md`)  
  详细设计，业务流程图 → 下一节点：GENHTML → 用户确认

- **06 GENHTML** (`nodes/07-genHTML.md`)  
  HTML原型生成，生成静态HTML原型 → 项目完成/进入CHANGE节点

- **07 CHANGE** (`nodes/08-change.md`)  
  变更分析，增量变更 → 回退到受影响节点
...
- `{项目名}_{datetime}/brainstorm/brainstorm.md` - 功能骨架
- `{项目名}_{datetime}/brainstorm/analysis.md` - brainstorm+clarify整合内容

### 纳入版本管理（output目录）
- `{项目名}_{datetime}/output/V{version}/PRD_{version}.md` - PRD文档
- `{项目名}_{datetime}/output/V{version}/prototype/index.html` - 原型索引
- `{项目名}_{datetime}/output/V{version}/prototype/app/page_*.html` - C端页面原型
- `{项目名}_{datetime}/output/V{version}/prototype/web/page_*.html` - B端页面原型

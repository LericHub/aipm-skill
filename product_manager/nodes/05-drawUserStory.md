...
### 加载 penpot-helper-skill

在绘制前，读取 `product_manager/skills/penpot-helper-skill/SKILL.md`，重点参考：
- `references/mcp-generation-guide.md`（坐标系统、ES5兼容）
- `references/user-story-layout-guide.md`（Flex布局防重叠）
- `references/layout-system.md`（Flex/Grid属性）

### 加载 penpot-drawing-skill

绘制时，加载 `product_manager/skills/penpot-drawing-skill/SKILL.md`，根据指引选择对应的绘制guide：
- 用户故事绘制 → `product_manager/skills/penpot-drawing-skill/references/user-story-drawing-guide.md`

### 加载 penpot-layout-optimizer

绘制完成后，若页面存在过多空白，读取 `product_manager/skills/penpot-layout-optimizer/SKILL.md` 进行布局优化。
...
> **前提**：用户已确认在浏览器中打开Penpot文件链接，且已读取 penpot-helper-skill 指引。
>

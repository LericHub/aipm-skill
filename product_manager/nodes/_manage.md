...
>
> **注意**：知识卡片加载已由 Router 统一处理，本文件不再包含加载步骤。如需加载知识卡片，请通过 Router 或调用 `load_relevant_knowledge_cards()` API。
...
```json
{
  ...
  "node_list": [
    ...
    {"name": "brainstorm", "status": "CONFIRM", "file": "项目名_YYYYMMDD_HHMMSS/analysis_process/brainstorm.md"},
    {"name": "clarify", "status": "CONFIRM", "file": "项目名_YYYYMMDD_HHMMSS/analysis_process/clarify.md"},
...
**说明**：
- `directories.analysis_process`：不纳入版本管理的analysis_process目录路径
...
- brainstorm和clarify节点的file指向analysis_process目录
